...
export function GameProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const helperFunc = async () => {
      ...
      console.log('sessionId found in gameCOntext which will be sent to syncWithServer is', sessionId)

      const res = await fetch(`${SERVER_URL || 'http://10.100.102.143:3000/'}sync-with-server`, {
        method: "POST", 
        headers: {
