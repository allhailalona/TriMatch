...
export default function GameBoard() {
  const { gameData, setGameData, userData, setUserData, isCheatModeEnabled, gameMode, totalSetsFound, setTotalSetsFound } = useGameContext();
  
  const { handleDrawACard, handleAutoFindSet } = useGameLogic()
    ...
    <StyledView className='h-full w-[94%] flex flex-row'>
      <StyledView className='w-[92%] h-full flex flex-row bg-purple-500'>
        <StyledView className='h-full w-[15%]'>
          <StyledText className='text-xl text-white font-bold'>{gameMode === '1' ? ('Whole stack') : ('3min Speed Run')}</StyledText>
          <StyledText className='text-xl text-white font-bold'>{totalSetsFound} sets found</StyledText>
          <StyledText className='text-xl text-white font-bold'>{gameMode === '1' ? ('stopwatch is running...') : ('3 min timer is running....')}</StyledText>
        </StyledView>
      ...
      <StyledView className='h-full w-[8%] flex flex-col justify-center bg-purple-500'>
        <StyledView className='h-[20%] bg-yellow-400 flex justify-center items-center rounded-lg'>
          <StyledView className='h-[50%] flex flex-row justify-between items-center p-2 px-4'>
          ...
          {isCheatModeEnabled && (
            <StyledView className='h-[50%] flex flex-row justify-between items-center p-2 px-4'>
              <StyledTouchableOpacity onPress={handleAutoFindSet}>
