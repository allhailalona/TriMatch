...
export default function StatsModal({ isOpen, onClose }: {isOpen: boolean, onClose: () => void}) {
  const { gameMode, setGameMode, isCheatModeEnabled, setIsCheatModeEnabled } = useGameContext()
  ...
  useEffect(() => {
    const loadSavedSettings = async (): Promise<void> => {
      try {
        ...
        if (savedGameMode) setGameMode(Number(savedGameMode))
  ...
  const handleChangeGameMode = async (newMode: 1 | 2): Promise<void> => {
    try {
      setGameMode(newMode)
       ...
       >
         ...
         <StyledView className="mb-4 border border-gray-200 rounded w-full">
           ...
           >
             <StyledView className="flex-row items-center flex-1">
               ...
               />
               <StyledText className="ml-2 text-lg">Whole Stack Speed Run</StyledText>
             </StyledView>
             <StyledText className="text-xl">{isAccordion1Open ? '▼' : '▶'}</StyledText>
           </StyledTouchableOpacity>
           
           {isAccordion1Open && (
             <StyledView className="p-3 border-t border-gray-200">
               <StyledText>Full stack marathon - Stopwatch runs until you complete all 81 cards. Race against yourself and track your best time!</StyledText>
             </StyledView>
           )}
         </StyledView>
         ...
         <StyledView className="mb-4 border border-gray-200 rounded w-full">
           ...
           >
             <StyledView className="flex-row items-center flex-1">
               ...
               />
               <StyledText className="ml-2 text-lg">3-Minute Challenge</StyledText>
             </StyledView>
             <StyledText className="text-xl">{isAccordion2Open ? '▼' : '▶'}</StyledText>
           </StyledTouchableOpacity>

           {isAccordion2Open && (
             <StyledView className="p-3 border-t border-gray-200">
               <StyledText>3-minute speedrun challenge - Complete as much as you can! Your final score is recorded when the timer hits zero.</StyledText>
             </StyledView>
           )}
         </StyledView>
         ...
         <StyledView className="flex-row justify-between items-center w-full mt-4">
           <StyledView className="flex-row items-center">
             ...
             />
             <StyledText className="ml-2 text-lg font-bold">Cheat Mode</StyledText>
           </StyledView>
           <StyledText className="text-lg font-bold text-blue-500 underline">DISCLAIMER</StyledText>
         </StyledView>
