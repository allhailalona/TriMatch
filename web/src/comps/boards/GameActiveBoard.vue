<template>
  <div class="w-[94%] h-full flex items-center justify-around flex-row bg-purple-600">
    ...
    <div class="h-full w-[5%] flex items-center">
      <div class="h-[20%] bg-yellow-500 flex flex-col justify-center items-center rounded-lg">
        <div class="h-[50%] flex flex-row gap-4 justify-between items-center p-2 px-4 hover:cursor-pointer" @click="drawACard">
          <OhVueIcon name="gi-card-draw" scale="2" fill="white"/>
        </div>
        <div v-if="cheatMode" 
              class="h-[50%] flex flex-row gap-4 justify-between items-center p-2 px-4 hover:cursor-pointer"
              @click="autoFindSet">
...
const { autoFindSet, drawACard } = useGameLogic()
