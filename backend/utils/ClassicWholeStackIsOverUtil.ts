...
import { autoFindSet } from "../gameLogic.js";
import type { SessionId, BoardFeed, ShuffledStack } from "../types.js";

export async function handleClassicGameIsOver(
  toReturn,
  sessionId: SessionId,
  boardFeed: BoardFeed[],
  userEmail: string,
  ...
  console.log(
    "gameMode IS classic checking length of shuffledStack and boardFeed",
  );
  const shuffledStack: ShuffledStack[] = await getGameState(
    `${sessionId}:shuffledStack`,
  );
  console.log(
    "length of boardFeed and shuffledStack is",
    boardFeed.length + shuffledStack.length,
  );

  let isGuest = false; // Default value

  // If there are still sets there is no need to stop the game
  if (shuffledStack.length + boardFeed.length <= 21) {
    // A game with more than 21 cards will ALWAYS contain a set
    console.log(
      "total available cards are below 21 - its possible that no more sets are present - checking with autoFindSet",
    );
    ...
    if (setFound) {
      // If the board has a set, the game is not over yet... dont start with the 'broken record' logic
      ...
      // Game Over!
      console.log(
        "a set wasnt found, declare game over - stopping stopwatch then checking for new records",
      );
      ...
      const startingTime: number = new Date(
        await getGameState(`${sessionId}:stopwatch`),
      ).getTime();
      ...
      if (userEmail) {
        ...
        console.log(
          "thats an id session, lets check if record was broken for user",
          userEmail,
        );
        ...
        if (prevRecord > newScore) {
          // If record was broken
          ...
          await UserModel.findByIdAndUpdate(userEmail, {
            "stats.speedrunWholeStack": newScore,
          });
        ...
        toReturn = {
          ...toReturn,
          /*no isRecordBroken since thats a guest session*/ newScore,
        }; // This time the user cannot break a record since he's a guest
      }
    ...
    // The game is not over yet there are still sets to find!
