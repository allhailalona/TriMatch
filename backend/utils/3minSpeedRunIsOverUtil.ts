...
import { io } from '../middelware/server'
...
export async function handleTimeIsUp(sessionId: string, userEmail: string) {
  ...
  if (!userEmail) { 
    ...
    await connect()
    const prevRecord = (await UserModel.findById(userEmail))!.stats.speedrun3min
    ...
    if (prevRecord < setsFound) {
      ...
      await UserModel.findByIdAndUpdate(userEmail, { "stats.speedrun3min": setsFound })
